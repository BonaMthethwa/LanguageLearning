...
router.get("zuluMultipleChoiceResults", (req , res) => {

    res.render("zuluMultipleChoiceResults");
   
  });

router.get("addMultipleChoiceOptions", (req , res) => {

    res.render("addMultipleChoiceOptions");
...
router.get('/filters', (req, res) => {
  ...
  router.get('/passedD', (req, res) => {
    // Query to fetch users who passed with distinction from scores table
    const query = 'SELECT user_email, score FROM scores WHERE score >= 13'; // Adjust the condition as per your scoring criteria
    ...
    database.query(query, (err, results) => {
        if (err) {
            ...
            const passedWithDistinction = results;

            // Render the hbs template with the fetched data
            res.render('passedD', { passedWithDistinction });
        }
    });
});
...
router.get('/passed', (req, res) => {
  // Query to fetch users who passed with distinction from scores table
  const query = 'SELECT user_email, score FROM scores WHERE score >= 9'; // Adjust the passing score criteria as per your application
